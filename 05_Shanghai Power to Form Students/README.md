...
python3 engine/build_report.py            # 三站全套 → out/<slug>/report.html + out/index.html
python3 engine/build_report.py caoyang    # 只跑单站
...
└─ engine/  (代码,不用进)
   ...
   ├─ plots/           画:每步一图 + 算子图谱
   ├─ build_report.py  出 out/<slug>/report.html(繁体);跑 python3 engine/build_report.py
   ├─ refresh_index.py 全扫 out/ 重建首页 index(维护用)
   └─ requirements.txt
```
