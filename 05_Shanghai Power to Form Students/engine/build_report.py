...
# 本脚本现位于 engine/。HERE=engine（取 common/report 等）；其父=05 根（取 config + data/out/yaml）。
_HERE = os.path.dirname(os.path.abspath(__file__)); _ROOT = os.path.dirname(_HERE)
sys.path.insert(0, _HERE); sys.path.insert(0, _ROOT)
os.chdir(_ROOT)   # data/ out/ yaml 等相对路径解析到 05 根；任意目录下 `python engine/build_report.py` 都可
import numpy as np
