"""
operators.py — 原子形态算子库(power 对 form 的「操作」)+ 配方引擎【进阶册】
=================================================================
...
教学要点:学生先学这 9 个操作,再学配方,再自己改 YAML / 复制粘贴换算子(见 算子替换指南.md)。
"""
...
def _center(recs, how):
    a = np.array([r["geom"].area for r in recs])
    h = np.array([r["h"] for r in recs])
    cx = np.array([r["geom"].centroid.x for r in recs])
    cy = np.array([r["geom"].centroid.y for r in recs])
    g = a * h
    if how == "state_centroid":
        m = np.array([r["sh"] == "state" for r in recs])
        if m.any() and g[m].sum() > 0:
            return float((cx[m] * g[m]).sum() / g[m].sum()), float(
                (cy[m] * g[m]).sum() / g[m].sum()
            )
...
def concentrate(
    ...
    d = np.array(
        [
            math.hypot(out[i]["geom"].centroid.x - Px, out[i]["geom"].centroid.y - Py)
            for i in idx
        ]
    )
...
def scale(recs, target, factor=2.0):
    ...
    for r in out:
        if _aff(r, target):
            r["geom"] = aff.scale(
                r["geom"], xfact=factor, yfact=factor, origin="centroid"
            )
    return out


def scale_graded(
    recs, target, center="global_centroid", factor=2.0, reach_frac=0.25, min_factor=1.0
):
...
def twist(recs, target, deg_per_100m=12.0):
    ...
    for r in out:
        if r["sh"] in target and not r.get("frozen"):
            d = math.hypot(r["geom"].centroid.x - Px, r["geom"].centroid.y - Py)
            r["geom"] = aff.rotate(
                r["geom"], deg_per_100m * d / 100.0, origin="centroid"
            )
    return out
...
OPS = {
    ...
    "split_to_towers": split_to_towers,
    "slim": slim,
    "densify": densify,
    "infill": infill,
    "level": level,
