"""prints for 02 映射:誰算誰的 —— 查表規則 / 一棟怎麼被判定 / 反事實前後 / 角色棟數。"""
import config
import common
...
def lookup_rules(lk=None):
    """印 shanghai_lookup.yaml 的級聯順序 + EULUC 用途→權利方主表 + 預設值。"""
...
def assign_trace(df, n=6, lk=None):
    """走查:前 n 棟的多源欄位 → assign_stakeholder 判出哪個角色(看它在哪一級命中)。"""
    lk = lk or common.load_lookup()
...
def counterfactual(res):
    """印 common.counterfactual() 的回傳:把某用途改判後,分布改前/改後。"""
