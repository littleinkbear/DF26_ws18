...
def loaded(df):
    """载入回报(02 开头那行)。"""
    _try(lambda: _say("载入 %d 栋  ·  %s" % (len(df), config.site_name())), "载入回报")


def lookup_rules(lk=None):
    """印 shanghai_lookup.yaml 的级联顺序 + EULUC 用途→权利方主表 + 预设值。"""
    lk = lk or common.load_lookup()
    _try(lambda: _say("级联顺序:", lk["cascade"]), "级联顺序")
    _say("\nEULUC 用途 → 权利方(主键,政商可分):")
    for k, v in lk.get("euluc", {}).items():
        _try(lambda k=k, v=v: _say("   %-10s -> %s" % (k, v)), "用途 %s" % k)
...
def counterfactual(res):
    ...
    _try(lambda: _say("把『%s』从 %s 改判给 %s:" % (res["flip"], res["frm"], res["to"])), "标题")
    _try(lambda: _say("  改前:", res["before"]), "改前分布")
    _try(lambda: _say("  改后:", res["after"]), "改后分布")


def stakeholders(df, label="各角色栋数"):
    """各角色栋数(value_counts)。02/03 共用。"""
    _try(lambda: _say("%s:" % label, df.stakeholder.value_counts().to_dict()), label)
